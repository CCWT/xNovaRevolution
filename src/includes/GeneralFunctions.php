...
function getFactors($USER, $PLANET, $Type = 'basic', $TIME = 0)
{
	...
	if ($Type == 'basic') {
		return array(
			array(
				'shipspeed' => 1 - $USER[$resource[706]],
				$TIME, $ExtraDM[701]['add'],
				0
			),
			array(
				'bulidspeed' => 1 - $USER[$resource[605]] * $OfficerInfo[605]['info'] - $USER[$resource[702]],
				$TIME, $ExtraDM[702]['add'],
				0,
				'techspeed' => 1 - $USER[$resource[606]] * $OfficerInfo[606]['info'] - $USER[$resource[705]],
				$TIME, $ExtraDM[705]['add'],
				0,
				...
				'metal' => 1 + ($USER[$resource[601]] * $OfficerInfo[601]['info']) + ($USER[$resource[131]] * 0.02) + $USER[$resource[703]],
				$TIME, $ExtraDM[703]['add'],
				0,
				'crystal' => 1 + ($USER[$resource[601]] * $OfficerInfo[601]['info']) + ($USER[$resource[132]] * 0.02) + $USER[$resource[703]],
				$TIME, $ExtraDM[703]['add'],
				0,
				'deuterium' => 1 + ($USER[$resource[601]] * $OfficerInfo[601]['info']) + ($USER[$resource[133]] * 0.02) + $USER[$resource[703]],
				$TIME, $ExtraDM[703]['add'],
				0,
				'energy' => 1 + ($USER[$resource[603]] * $OfficerInfo[603]['info']) + $USER[$resource[704]],
				$TIME, $ExtraDM[704]['add'],
				0,
			),
		);
	}
	if ($Type == 'attack') {
		return array(
			'attack' => $USER[$resource[602]] * $OfficerInfo[602]['info'] + $USER[$resource[700]],
			$TIME, $ExtraDM[700]['add'],
			0,
			'defensive' => $USER[$resource[602]] * $OfficerInfo[602]['info'] + $USER[$resource[701]],
			$TIME, $ExtraDM[701]['add'],
			0,
